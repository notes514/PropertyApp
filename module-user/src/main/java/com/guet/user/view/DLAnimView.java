...
public class DLAnimView extends View {
        ...
        for (int i = 0; i < POINT; i++) {
            for (int j = 0; j < POINT; j++) {
                if (i + j < POINT) {
                    ...
                    if (lineOpacity > 0) {
                        @SuppressLint("DrawAllocation") Line line =
                                new Line(circles.get(i).x, circles.get(i).y, circles.get(i + j).x,
                                        circles.get(i + j).y, lineOpacity);
    ...
    private void init() {
        ...
        for (int i = 0; i < POINT; i++) {
            circles.add(new Circle(num(width, 0), num(height, 0), num(15, 2),
                    num(10, -10) / 40, num(10, -10) / 40));
        }
    }
